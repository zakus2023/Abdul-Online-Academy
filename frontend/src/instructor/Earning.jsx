...
function Earning() {
  ...
  const [earning, setEraning] = useState([]);
  const [bestSelling, setBestSelling] = useState([]);

  const fetchMonthlyEarnings = async () => {
    const res = await useAxios().get(
      `teacher/monthly-earnings/${UserData()?.teacher_id}/`
    );
    setEraning(res.data);
  };
  ...
  const fetchTeacherSummary = async () => {
    const res = await useAxios().get(
      `teacher/summary/${UserData()?.teacher_id}/`
    );
  ...
  const fetchBestSellingCourse = async () => {
    const res = await useAxios().get(
      `teacher/best-selling-course/${UserData()?.teacher_id}/`
    );
                          ...
                          <td>
                            ...
                            >
                              <div className="d-flex align-items-center">
                                <img
                                  src={`http://127.0.0.1:8000/${bs.course_mage}`}
                                  alt="course"
                                  style={{
                                    width: "100px",
                                    height: "70px",
